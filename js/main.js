...
function showOnlyLogin() {
    ...
    if (JSON.parse(localStorage.getItem('locations')) == null) {
        ...
        JSON.parse(localStorage.getItem('locations')).forEach(location => {
            ...
            const url = "http://nominatim.openstreetmap.org/search?street=" + location.number + "+" + location.address +
                "&postalcode=" + location.postcode + "&format=json";
            ...
            httpRequest.onload = function (e) {
                ...
                if (this.status = 200 && obj[0] != undefined) {
                    addToLocations(obj[0]["lat"], obj[0]["lon"], location.name, location.address, location.number, location.postcode);
                } else {
...
updateLocation.addEventListener("submit", (e) => {
    ...
    if (index != -1) {
        ...
        let httpRequest = new XMLHttpRequest();
        const url = "http://nominatim.openstreetmap.org/search?street=" + number + "+" + address + "&postalcode=" + plz + "&format=json";
        ...
        httpRequest.onload = function (e) {
            ...
            if (this.status = 200 && obj[0] != undefined) {
                addToLocations(obj[0]["lat"], obj[0]["lon"], name, address, number, plz);
                document.getElementById("mainpage").style.display = "block";
                document.getElementById("updateDelete").style.display = "none";
            } else {
                alert("Please check the address!")
